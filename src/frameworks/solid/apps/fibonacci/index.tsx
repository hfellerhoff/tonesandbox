import Input from "@solid/components/Input";
...
export default function FibonacciPage() {
  ...
  const [notes, setNotes] = createSignal<string[]>(
    generateNotes(numberOfNotes(), mod())
  );
  ...
  const renderScore = () => {
    ...
    const topOffset =
      extraOctavesAbove > extraOctavesBelow
        ? (extraOctavesAbove - extraOctavesBelow) * EXTRA_OCTAVE_HEIGHT
        : 0;
    ...
    const [bassNotes, trebleNotes] = notes().reduce(
      ([bassAcc, trebleAcc], note) => {
      ...
      <div class="mt-4 max-w-sm mx-auto w-full px-4">
        ...
        <div class="grid place-items-center mt-4">
          <button
            ...
            class="w-full py-3 px-4 font-medium rounded active:translate-y-0.5 flex items-center justify-center gap-2 text-sm bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100"
          >
            {isPlaying() ? "Stop" : "Play"}
          </button>
        </div>
